...
public class EcoreCreationWizardPage extends WizardPage {
	...
	private boolean validateNewModelGroup() {
		...
		if (container == null || container instanceof IWorkspaceRoot) {
			setErrorMessage(org.eclipse.emf.ecoretools.legacy.diagram.Messages.EcoreCreationWizardPage_UnvalidDestinationContainer);
			return false;
		}

		if (getFileName() == null || "".equals(getFileName())) { //$NON-NLS-1$
			setErrorMessage(org.eclipse.emf.ecoretools.legacy.diagram.Messages.EcoreCreationWizardPage_DomainFileNotEmpty);
			return false;
	...
	private void validateExistingFiles() {
		if (modelFileExist()) {
			if (diagramFileExist()) {
				setMessage(org.eclipse.emf.ecoretools.legacy.diagram.Messages.EcoreCreationWizardPage_FilesAlreadyExist, IMessageProvider.WARNING);
			} else {
				setMessage(org.eclipse.emf.ecoretools.legacy.diagram.Messages.EcoreCreationWizardPage_DomainFileAlreadyExists, IMessageProvider.WARNING);
			}
		} else if (diagramFileExist()) {
			setMessage(org.eclipse.emf.ecoretools.legacy.diagram.Messages.EcoreCreationWizardPage_DiagramFileAlreadyExists, IMessageProvider.WARNING);
		}
	}
	...
	private boolean validateExistingModel() {
		...
		if (modelFd.getText() == null || "".equals(modelFd.getText())) { //$NON-NLS-1$
			setErrorMessage(org.eclipse.emf.ecoretools.legacy.diagram.Messages.EcoreCreationWizardPage_ChooseExistingDomainFile);
			return false;
		}
		...
		if (!(getDiagramEObject() instanceof EPackage)) {
			setErrorMessage(org.eclipse.emf.ecoretools.legacy.diagram.Messages.EcoreCreationWizardPage_ChooseAnEPackageElement);
			return false;
		}
